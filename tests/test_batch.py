...
class Batch(unittest.TestCase):
    ...
    def test_docstrings_count_the_current_marks_and_link_to_who_gave_them(self):
        text = docstrings(INDEX, [ALICE, BOT, OLD], SITE)
        self.assertIn("/-- The function `f`.\n\n"
                      "Reviewed-by: 1 person ([who](https://example.org/reviews/#d=TauCeti.X.f))\n"
                      "Tested-by: 1 AI agent ([who](https://example.org/reviews/#d=TauCeti.X.f)) -/\ndef f : ℕ := 1", text)
    ...
    def test_many_marks_still_take_one_line_each(self):
        many = [dict(ALICE, by=f"person{n}") for n in range(12)] + [dict(BOT, trailer="Reviewed-by", agent=f"Agent {n}") for n in range(5)]
    ...
    def test_the_table_counts_each_mark(self):
        ...
        self.assertIn("| [`TauCeti.X.f`](https://example.org/reviews/#d=TauCeti.X.f) | 1 person | 1 AI agent |", text)
